...
(function () {
  ...
  function init() {
    try {
      S.videoPath = String(window.PREPROC_VIDEO || "");
    } catch (e) {
      S.videoPath = "";
    }
    var v = U.$("#pp-video");
    ...
    function _persistTab(name) {
      try {
        ...
        try {
          localStorage.setItem("cheesepie.preproc.step", String(name || ""));
        } catch (e) {}
      } catch (e) {}
    }
    ...
    function switchTab(name) {
      ...
      overlay.style.pointerEvents =
        (name === "arena" && S.marking) || name === "colors" ? "auto" : "none";
      _persistTab(name);
      try {
        document.dispatchEvent(
          new CustomEvent("preproc:tab-changed", { detail: { name: name } })
        );
      } catch (e) {}
      try {
    ...
    if (tabs.info)
      tabs.info.addEventListener("click", function () {
        if (tabs.info.disabled) return;
        switchTab("info");
      });
    if (tabs.arena)
      tabs.arena.addEventListener("click", function () {
        if (tabs.arena.disabled) return;
        switchTab("arena");
      });
    if (tabs.timing)
      tabs.timing.addEventListener("click", function () {
        if (tabs.timing.disabled) return;
        switchTab("timing");
      });
    if (tabs.background)
      tabs.background.addEventListener("click", function () {
        if (tabs.background.disabled) return;
        switchTab("background");
      });
    if (tabs.regions)
      tabs.regions.addEventListener("click", function () {
        if (tabs.regions.disabled) return;
        switchTab("regions");
      });
    if (tabs.colors)
      tabs.colors.addEventListener("click", function () {
        if (tabs.colors.disabled) return;
        switchTab("colors");
      });
    if (tabs.timing)
      tabs.timing.addEventListener("click", function () {
    ...
    if (tabs.save)
      tabs.save.addEventListener("click", function () {
        if (tabs.save.disabled) return;
        switchTab("save");
      });
    ...
    if (!S.videoPath) {
      try {
        ...
        ph.className = "placeholder muted";
        ph.textContent =
          "No video selected. Choose a file from Browser to begin.";
        if (panes.arena) panes.arena.insertBefore(ph, panes.arena.firstChild);
      } catch (e) {}
    } else {
      // Load video source
      try {
        v.src = "/media?path=" + encodeURIComponent(S.videoPath);
        v.load();
      } catch (e) {}
    }
    ...
    (function wireExport() {
      ...
      btn.addEventListener("click", function () {
        try {
          ...
          var base = S.videoPath
            ? S.videoPath.split("/").pop() || "video"
            : "frame";
    ...
    function setTabsEnabled(enabled) {
      try {
        ...
        var hasSetup = !!(
          document.getElementById("pp-setup") &&
          document.getElementById("pp-setup").value
        );
        ...
        Object.keys(tabs).forEach(function (k) {
          ...
          if (
            k === "colors" &&
            !(
              (Preproc && Preproc.State && Preproc.State.hasBackground) ||
              false
            )
          ) {
            tabs[k].disabled = true;
    ...
    function applyRegionsEditingGating() {
      try {
        if (isRegionsEditing) {
          ...
          Object.keys(tabs).forEach(function (k) {
            if (!tabs[k]) return;
            if (k !== "regions" && k !== "arena") {
              tabs[k].disabled = true;
              tabs[k].title = "Finish editing the region first";
            }
          });
        } else {
    ...
    function updateUrlWithFacility(fac) {
      try {
        var url = new URL(window.location.href);
        if (fac) url.searchParams.set("facility", fac);
    ...
    document.addEventListener("app:facility-changed", function (ev) {
      try {
        var fac =
          (ev && ev.detail && ev.detail.name) ||
          (facilitySel && facilitySel.value) ||
          "";
        updateUrlWithFacility(fac);
        setTabsEnabled(!!fac);
        if (fac) {
          populateSetupsForFacility(fac);
        }
      } catch (e) {}
    });
      ...
      if (Array.isArray(raw)) {
        ...
        raw.forEach(function (su) {
          ...
          if (pp.arena_tl && pp.arena_br) {
            try {
              var ax = parseInt(pp.arena_tl.x) || 0,
                ay = parseInt(pp.arena_tl.y) || 0;
              var bx = parseInt(pp.arena_br.x) || ax,
                by = parseInt(pp.arena_br.y) || ay;
              rect = {
                x: ax,
                y: ay,
                width: Math.max(0, bx - ax),
                height: Math.max(0, by - ay),
              };
            } catch (e) {}
          }
    ...
    function populateSetupsForFacility(fac) {
      ...
      if (!setups) {
        ...
        if (legacy && legacy.length) {
          setups = {
            default: {
              roi: legacy.reduce(function (m, it) {
                try {
                  m[it.name || "roi"] = {
                    cells: it.cells || [],
      ...
      try {
        var key = "cheesepie.preproc.setup." + String(fac || "");
        var saved =
          localStorage.getItem(key) ||
          localStorage.getItem("cheesepie.preproc.setup") ||
          "";
    ...
    function applySetupDefaults(fac, name, setups) {
      try {
        ...
        if (wcm && pp.arena_width_cm != null)
          wcm.value = String(pp.arena_width_cm);
        if (hcm && pp.arena_height_cm != null)
          hcm.value = String(pp.arena_height_cm);
        if (bgf && pp.bg_frames != null) bgf.value = String(pp.bg_frames);
        if (bgq && pp.bg_quantile != null) bgq.value = String(pp.bg_quantile);
        if (
          pp.arena &&
          typeof pp.arena.x === "number" &&
          typeof pp.arena.y === "number" &&
          typeof pp.arena.width === "number" &&
          typeof pp.arena.height === "number"
        ) {
          S.tl = { x: pp.arena.x | 0, y: pp.arena.y | 0 };
          S.br = {
            x: (pp.arena.x + pp.arena.width) | 0,
            y: (pp.arena.y + pp.arena.height) | 0,
          };
        }
        if (arena && arena.drawOverlay) arena.drawOverlay();
      } catch (e) {}
    ...
    if (setupSel) {
      setupSel.addEventListener("change", function () {
        ...
        /* do not auto-apply */ try {
          localStorage.setItem(
            "cheesepie.preproc.setup." + String(fac || ""),
            setupSel.value || ""
          );
          localStorage.setItem("cheesepie.preproc.setup", setupSel.value || "");
        } catch (e) {}
        setTabsEnabled(true);
      });
    }
    if (applySetupBtn) {
      applySetupBtn.addEventListener("click", function () {
        var fac = facilitySel ? facilitySel.value : "";
        if (!fac) {
        ...
        applySetupDefaults(fac, setupSel && setupSel.value, currentSetups);
      });
    }
    if (saveSetupBtn) {
      saveSetupBtn.addEventListener("click", function () {
        ...
        if (!window.confirm("Are you sure you want to save this setup?"))
          return;
            ...
            arena:
              S.tl && S.br
                ? {
                    x: S.tl.x,
                    y: S.tl.y,
                    width: Math.max(0, S.br.x - S.tl.x),
                    height: Math.max(0, S.br.y - S.tl.y),
                  }
                : null,
            roi: {},
          },
        ...
        fetch("/api/preproc/setup/save", {
          ...
          .then(function (r) {
            return r.json().then(function (d) {
              return { ok: r.ok, d: d, status: r.statusText };
            });
          })
          .then(function (res) {
            if (!res.ok || res.d.error) {
              alert("Save failed: " + ((res.d && res.d.error) || res.status));
              return;
            }
            populateSetupsForFacility(fac);
            alert("Saved.");
    ...
    if (saveFinalBtn) {
      saveFinalBtn.addEventListener("click", function () {
        var videoPath =
          (window.Preproc &&
            window.Preproc.State &&
            window.Preproc.State.videoPath) ||
          "";
        if (!videoPath) {
    ...
    var regions = null;
    if (Preproc.Regions) regions = Preproc.Regions.init({});
    try {
      Preproc.__regions = regions;
    } catch (e) {}
    var bg = null;
    if (Preproc.Background) bg = Preproc.Background.init({});
    if (Preproc.Colors) Preproc.Colors.init({});
    // Save button gating: background + 10 marks per mouse (R,G,B,Y)
    (function wireSaveGating() {
      try {
        var saveBtn = U.$("#pp-open-save");
        if (!saveBtn) return;
        ...
        function meets() {
          try {
            var bgOk = !!(
              Preproc &&
              Preproc.State &&
              Preproc.State.hasBackground
            );
            var c = Preproc.State.mouseCounts || {};
            return bgOk && c.R >= 10 && c.G >= 10 && c.B >= 10 && c.Y >= 10;
          } catch (e) {
            return false;
          }
        }
        function update() {
          try {
            saveBtn.classList.toggle("success", !!meets());
          } catch (e) {}
        }
        document.addEventListener("preproc:background-ready", function () {
          try {
            Preproc.State.hasBackground = true;
            update();
          } catch (e) {}
        });
        document.addEventListener("preproc:colors-counts", function (ev) {
          try {
            var d = ev && ev.detail && ev.detail.counts;
            if (d) {
              Preproc.State.mouseCounts = d;
              update();
            }
          } catch (e) {}
        });
    ...
    if (facilitySel && facilitySel.value) {
      populateSetupsForFacility(facilitySel.value);
    }
    setTabsEnabled(true);
    try {
      ...
      var step0 =
        usp0.get("step") ||
        localStorage.getItem("cheesepie.preproc.step") ||
        "" ||
        "info";
        ...
        fetch("/api/preproc/state?video=" + encodeURIComponent(S.videoPath))
          ...
          .then(function (d) {
            ...
            try {
              ...
              if (facVal) {
                if (facilitySel) {
                  facilitySel.value = facVal;
                }
                // Persist and populate setups for this facility
                try {
                  localStorage.setItem("cheesepie.preproc.facility", facVal);
                } catch (e) {}
                ...
                if (setupVal && document.getElementById("pp-setup")) {
                  ...
                  try {
                    localStorage.setItem(
                      "cheesepie.preproc.setup." + String(facVal),
                      setupVal
                    );
                    localStorage.setItem("cheesepie.preproc.setup", setupVal);
                  } catch (e) {}
                }
                setTabsEnabled(true);
              }
            ...
            try {
              ...
              setVal(
                "meta-fps",
                vi.frame_rate != null
                  ? Number(vi.frame_rate).toFixed(2) + " fps"
                  : "—"
              );
              setVal("meta-duration", fmtDur(Number(vi.duration || 0)));
              setVal(
                "meta-frames",
                vi.num_frames != null ? vi.num_frames : "—"
              );
            } catch (e) {}
            ...
            if (d.arena) {
              ...
              try {
                var colsEl = U.$("#grid-cols"),
                  rowsEl = U.$("#grid-rows"),
                  wcmEl = U.$("#arena-wcm"),
                  hcmEl = U.$("#arena-hcm");
                if (colsEl && d.arena.grid_cols != null)
                  colsEl.value = String(d.arena.grid_cols);
                if (rowsEl && d.arena.grid_rows != null)
                  rowsEl.value = String(d.arena.grid_rows);
                if (wcmEl && d.arena.width_in_cm != null)
                  wcmEl.value = String(d.arena.width_in_cm);
                if (hcmEl && d.arena.height_in_cm != null)
                  hcmEl.value = String(d.arena.height_in_cm);
              } catch (e) {}
            ...
            try {
              if (d.meta && typeof d.meta === "object") {
                ...
                if (
                  stEl &&
                  typeof d.meta.start_time === "string" &&
                  d.meta.start_time
                ) {
                  stEl.value = d.meta.start_time;
                }
                if (
                  enEl &&
                  typeof d.meta.end_time === "string" &&
                  d.meta.end_time
                ) {
                  enEl.value = d.meta.end_time;
                }
              }
            } catch (e) {}
            if (d.background) {
                ...
                if (bgCanvas) {
                  ...
                  img.onload = function () {
                    try {
                      ...
                      try {
                        document.dispatchEvent(
                          new CustomEvent("preproc:background-ready")
                        );
                      } catch (e) {}
                    } catch (e) {}
                  };
                  ...
                  try {
                    ...
                    Object.keys(frames0 || {}).forEach(function (k) {
                      var ms = (frames0[k] && frames0[k].marks) || [];
                      ms.forEach(function (m) {
                        var mm = m && m.mouse;
                        if (countsInit[mm] != null) countsInit[mm]++;
                      });
                    });
                    Preproc.State.mouseCounts = countsInit;
                    document.dispatchEvent(
                      new CustomEvent("preproc:colors-counts", {
                        detail: { counts: countsInit },
                      })
                    );
                  } catch (e) {}
                }
    ...
    document.addEventListener("preproc:arena-changed", function () {
      if (!isRegionsEditing)
        setTabsEnabled(!!(facilitySel && facilitySel.value));
    });
    document.addEventListener("preproc:regions-editing", function (ev) {
      isRegionsEditing = !!(ev && ev.detail && ev.detail.editing);
      applyRegionsEditingGating();
    });
