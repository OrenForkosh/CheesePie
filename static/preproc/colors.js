...
(function(){
  ...
  function init(){
    ...
    document.addEventListener('keydown',(ev)=>{ if(!pane||pane.style.display==='none') return; const k=ev.key; if(k==='1') setMouse('R'); else if(k==='2') setMouse('G'); else if(k==='3') setMouse('B'); else if(k==='4') setMouse('Y'); else if(k==='0') setMouse('BG'); });
      ...
      try{
        ...
        defs.forEach((d, i) => {
          ...
          function onDown(ev){
            ...
            ghost.style.position = 'absolute'; ghost.style.left = (hostRect.left)+'px'; ghost.style.top = (hostRect.top)+'px'; ghost.style.width = '0'; ghost.style.height='0'; ghost.style.zIndex='11'; ghost.style.pointerEvents='none';
            ...
            drag.style.left = (elRect.left)+'px'; drag.style.top = (elRect.top)+'px';
          ...
          function onUp(ev){ if (!dragging) return; dragging=false; el.style.cursor='grab';
            ...
            try{
              ...
              // Compute centroid and add mark for this mouse id
              let sumx=0,sumy=0,cnt=0; for(let yy=0;yy<H;yy++){ const row=lastIndex[yy]||[]; for(let xx=0;xx<W;xx++){ if ((row[xx]|0)===lab){ sumx+=xx; sumy+=yy; cnt++; } } }
              const cx = cnt? (sumx/cnt) : lx; const cy = cnt? (sumy/cnt) : ly;
              ...
              drawMarks(); updateIndicator(); setStatus(`Assigned ${d.id} to segment ${lab}`);
              cleanup(true);
    ...
    // Toggle dock visibility with tab changes
    document.addEventListener('preproc:tab-changed', function(ev){
      try{
        ...
        if (!dock) return;
        dock.style.display = (name === 'colors') ? 'flex' : 'none';
      } catch(e){}
    });
    ...
    async function run(){ const snap=snapshotDataURL(); const dataUrl=snap.frame; if(!dataUrl){ setStatus('Video not ready'); return;} setStatus('Segmenting…'); try{ await ensureSavedFrames(); let background=null; try{ const bg=document.getElementById('bg-canvas'); if(bg&&bg.width&&bg.height) background=bg.toDataURL('image/png'); }catch(e){} const resp=await fetch('/api/preproc/segment_simple',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ image:dataUrl, background })}); const data=await resp.json(); if(!resp.ok||!data||!data.ok){ setStatus('Error: '+(data&&data.error||resp.statusText)); return;} if(data.stats&&typeof data.stats.nonzero==='number'){ if(data.stats.nonzero===0) setStatus('No segments detected (try a different frame or compute background)'); else setStatus(`Segments detected: ${data.stats.unique ? data.stats.unique.length-(data.stats.unique.includes(0)?1:0):''}`);} if(Array.isArray(data.index)&&data.index.length){ lastIndex=data.index; lastSize={h:data.index.length|0,w:(data.index[0]||[]).length|0}; } const overlayImg=data.overlay_b64; if(!overlayImg){ setStatus('No overlay returned'); return;} const vw=v&&v.clientWidth?v.clientWidth:(processed&&processed.clientWidth)||0; const vh=v&&v.clientHeight?v.clientHeight:(processed&&processed.clientHeight)||0; if(!vw||!vh){ setStatus('Video panel not ready'); return;} const img=new Image(); await new Promise(res=>{ img.onload=()=>res(); img.onerror=()=>res(); img.src=overlayImg;}); processed.width=vw; processed.height=vh; processed.style.display=''; const pctx=processed.getContext('2d'); pctx.imageSmoothingEnabled=false; pctx.clearRect(0,0,vw,vh); const r=fitRect(img.naturalWidth||img.width, img.naturalHeight||img.height, vw, vh); pctx.drawImage(img,r.dx,r.dy,r.dw,r.dh); syncFromSaved(); drawMarks(); updateIndicator(); setStatus(''); }catch(e){ setStatus('Error: '+e); } }
    ...
    tab?.addEventListener('click', async ()=>{ if(pane) pane.style.display=''; await ensureSavedFrames(); syncFromSaved(); drawMarks(); updateIndicator(); run(); });
    v?.addEventListener('seeked', ()=>{ if(pane&&pane.style.display!=='none'){ syncFromSaved(); drawMarks(); updateIndicator(); run(); } });

    overlay?.addEventListener('click', (ev)=>{ try{ if(!lastIndex){ setStatus('Run segmentation first'); return;} const rect=overlay.getBoundingClientRect(); const mx=ev.clientX-rect.left, my=ev.clientY-rect.top; const vw=overlay.clientWidth, vh=overlay.clientHeight; const W=lastSize.w,H=lastSize.h; const r=fitRect(W,H,vw,vh); if(mx<r.dx||my<r.dy||mx>r.dx+r.dw||my>r.dy+r.dh){ setStatus('Click inside the video area'); return;} const lx=Math.round((mx-r.dx)*(W/r.dw)); const ly=Math.round((my-r.dy)*(H/r.dh)); const labelAt=(x,y)=>{ if(y<0||y>=H||x<0||x>=W) return 0; const row=lastIndex[y]||[]; return row[x]|0; }; let lab=labelAt(lx,ly); if(lab===0){ const rad=2; outer: for(let dy=-rad;dy<=rad;dy++){ for(let dx=-rad;dx<=rad;dx++){ const l2=labelAt(lx+dx,ly+dy); if(l2>0){ lab=l2; break outer; } } } } if(lab===0 && currentMouse!=='BG'){ setStatus('No segment here'); return;} let sumx=0,sumy=0,cnt=0; for(let y=0;y<H;y++){ const row=lastIndex[y]||[]; for(let x=0;x<W;x++){ if((row[x]|0)===lab){ sumx+=x; sumy+=y; cnt++; } } } const cx=cnt?(sumx/cnt):lx; const cy=cnt?(sumy/cnt):ly; const t=timeKey(); if(!marks[t]) marks[t]=[]; marks[t]=marks[t].filter(m=>m.segment_label!==lab); marks[t].push({ mouse:currentMouse, segment_label:lab, centroid:{x:cx,y:cy} }); drawMarks(); updateIndicator(); setStatus(`Marked ${currentMouse} on segment ${lab}`); }catch(e){ setStatus('Error: '+e); } });

    async function saveMarks(){ try{ if(!lastIndex){ setStatus('Run segmentation first'); return;} const t=timeKey(); const list=marks[t]||[]; if(!list.length){ setStatus('No marks to save'); return;} const snap=snapshotDataURL(); if(!snap.frame){ setStatus('Video not ready'); return;} const frameObj={ image_b64:snap.frame, labels:lastIndex, marks:list.map(m=>({ mouse:m.mouse, segment_label:m.segment_label, centroid:m.centroid })) }; const body={ video:(window.Preproc&&window.Preproc.State&&window.Preproc.State.videoPath)||'', colors:{ frames:{ [t]: frameObj } } }; const resp=await fetch('/api/preproc/colors',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)}); const data=await resp.json(); if(!resp.ok||!data||!data.ok){ setStatus('Save failed: '+(data&&data.error||resp.statusText)); return;} await ensureSavedFrames(); savedFrames[t]=frameObj; setStatus('Saved'); }catch(e){ setStatus('Error: '+e); } }
    saveBtn?.addEventListener('click', saveMarks);
    clearBtn?.addEventListener('click', ()=>{ const t=timeKey(); marks[t]=[]; drawMarks(); setIndicator(''); if(listEl) listEl.style.display='none'; setStatus('Cleared'); });
